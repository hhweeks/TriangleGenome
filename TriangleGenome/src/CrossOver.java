 ...
 *
 * @author Paige Romero
 * @author Hans Weeks This static object is used for single point cross over.
 *         The main method (breed) accepts 4 Genomes as arguments, making the
 *         first 2 parents and the latter 2 children. Breed copies genes from
 *         one parent up to the cross over point, calls geneSplice to create a
 *         gene consisting of both parents, then copies the rest of the genes
 *         from the second parent.
 ****************************************************************************/
public class CrossOver {
	/****************************************************************************
	 * breed Input:4 genomes, the first 2 parent the second 2 and an integer
	 * between 0 and 2000 for a crossover TODO make crossover above 10
	 * Output:none Description:does single point cross over. Clears the genes of
	 * the child nodes before adding parent genes (meaning children can be
	 * to-recycle Genomes)
	 ****************************************************************************/
	...
	public static void multiBreed(Genome papaGenome, Genome mamaGenome,
		...
		breed(papaGenome, mamaGenome, sonGenome, daughterGenome, crossPoint);
		for (int i = 0; i < NPoints; i++) {
			crossPoint = rand.nextInt(2000);
			breed(GenomeUtilities.genomeCopy(sonGenome),
					GenomeUtilities.genomeCopy(daughterGenome), sonGenome,
					daughterGenome, crossPoint);
	...
	public static void breed(Genome papaGenome, Genome mamaGenome,
		...
		daughterGenome.geneList.addAll(mamaGenome.geneList
				.subList(0, genecross));
		...
		for (int i = genecross + 1; i < papaGenome.geneList.size(); i++) {
			Gene tempGene = GenomeUtilities
					.geneCopy(papaGenome.geneList.get(i));
	...
	/****************************************************************************
	 * breed Input: 3 genomes, the first 2 parents, the last the child gene. Top
	 * parent is the parent whose genes appear first in the child's list
	 * (papaGenome is top parent for son, mamaGenome is top parent for daughter.
	 * Output:returns the spliced gene consisting of top and bottom parents'
	 * alleles Description:puts the 2 parents alleles (gene data) into a 2D
	 * array, builds an array with the information from both, switching at the
	 * withinGeneSplit, then assigns these values to a new gene, which is then
	 * returned.
	 ****************************************************************************/
	public static Gene geneSplice(Gene topParent, Gene bottomParent,
			int crossPoint) {
		...
		{
			for (int j = 0; j < genArr[i].NPOINTS; j++)// get xpoints and
														// ypoints, 3
														// of each and add them
														// to the
														// array
			{
		...
		// ///////////////////////////////////
		for (int j = 0; j < childSplicedGene.NPOINTS; j++)// get xpoints and
															// ypoints, 3 of
															// each and add them
															// to the array
		{
