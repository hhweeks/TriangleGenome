...
public class HillClimber extends Thread
{
  ...
  /****************************************************************************
   * Constructor
   * Input: BufferedImage master image, Genome to hillclimb, int steps, number of
   * times to mutate before calling crossover
   ****************************************************************************/
   ...
   * Input:none
   * Output:none
   * Description: calls climbStep to mutate a genes in the Genome nSteps times.
   *  When climb is done being called, sets a flag to tell the Tribe to crossover
   ****************************************************************************/
  public void run()
  {
    gradientClimb = false;
  ...
  /****************************************************************************
   * climbSte
   * Input:Genome to modify
   * Output:boolean did the fitness improve?
   * Description:selects a Gene to mutate, selects an element "allele" to mutate,
   *  selects an amount to mutate that gene, tests to make sure the resulting
   *  value of the allele would still be in bounds. then calls mutate and helper
   *  functions to either repeat the mutation (if lowered fitness score) or to undo
   *  the mutation if it raised fitness score.
   ****************************************************************************/
    ...
    if (!gradientClimb)
    {
      ...
      } else
      {
        ...
        if (endScoreLocal > startScoreLocal)
        {
          ...
          repeatMutation(myGenome, lastGene, lastAllele, lastShift, maxBound,
              startScoreLocal, endScoreLocal);
  ...
  /****************************************************************************
   * reverGenome 
   * Input:Gene to undu, allele modified and amount modified by
   * Output:none
   * Description:helper function to undo the mutation on a Gene after a bad mutation
   ****************************************************************************/
  ...
  /****************************************************************************
   * repeatMutation
   * Input:requires all the variable used to make the mutation including the Gene,
   *  containing Genome, the allele, how much it was mutated, its start and end scores,
   *  the amount shifted, and the maximum amount the allele can be shifted before 
   *  going out of bounds
   * Output:none
   * Description: mutates the Gene in a loop until the mutation no longer improves its
   *  fitness, then removes the last mutation (which was harmful)
   ****************************************************************************/
   ...
   * Input:Genome to modify
   * Output:none
   * Description:unused in turned-in version. This method served to calculate
   *  how quickly fitness improved with modifications to each gene, and use that
   *  to direct mutation
   ****************************************************************************/
  ...
  /****************************************************************************
   * shiftGene 
   * Input:currentGene, allele to mod in Gene and how much to shift
   * Output:none
   * Description:helper function of Gradient to modify alleles
   ****************************************************************************/
...
//  }
  ...
  /****************************************************************************
   * getAllele 
   * Input:current Gene to select from
   * Output:int, represents element selected
   * Description:selects random element to change in the allele
   ****************************************************************************/
  public int getAllele(Gene myGene)
  {
    int myAlleleIndex = rand.nextInt(Gene.NALLELE + 2);// 10 alleles+2 for move all x and move all y
    return myAlleleIndex;
  }

  /****************************************************************************
   * switchLayers
   * Input:2 genes to switch
   * Output:none
   * Description:switches the values of 2 genes by copying their data
   ****************************************************************************/
  ...
  /****************************************************************************
   * assignScore
   * Input:Genome to score
   * Output:fitness score, long
   * Description:calls scaled scoring on the genome, which ocurrs in many places
   *  in this class and so is centralized here.
   ****************************************************************************/
  ...
  /****************************************************************************
   * getLocalFit
   * Input:Gene, allele and amount shifted
   * Output:long fitness score
   * Description:used to calculate fitness score of a small peice of the image
   *  not used in the completed version of the project
   ****************************************************************************/
